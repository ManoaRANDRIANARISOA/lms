...
const zeroPaymentsAndMultipleYears = fees.filter(f => f.payment_count === 0 && f.total_fee_records > 1);
...
db.transaction(() => {
  for (const record of zeroPaymentsAndMultipleYears) {
    console.log(`Deleting fee record ${record.fee_id} for student ${record.first_name} ${record.last_name}`);
