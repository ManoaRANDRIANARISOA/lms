...
export function registerGradeHandlers(): void {
  ...
  ipcMain.handle('grade:updateSubject', async (_, id: string, updates: any) => {
    if (!canWrite('grades')) {
      return { success: false, error: 'Accès refusé' }
    }
    try {
  ...
  // GRADES
  // --------------------------------------------
  ipcMain.handle('grade:createGrade', async (_, data: any) => {
    if (!canWrite('grades')) {
      return { success: false, error: 'Accès refusé' }
    }
    try {
      const result = GradeRepository.createGrade(data)
      if (result.success) {
  ...
  ipcMain.handle('grade:updateGrade', async (_, id: string, updates: any) => {
    if (!canWrite('grades')) {
      return { success: false, error: 'Accès refusé' }
    }
    try {
